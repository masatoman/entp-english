...
function getEnemyTypeForWave(wave: number): EnemyType {
  ...
  if (wave <= 2) {
    ...
    // 高ウェーブ：全種類登場
    const types: EnemyType[] = ['basic', 'fast', 'tank', 'regen', 'shield', 'stealth'];
...
export function calculateEnemyStats(wave: number): {
  ...
  const health = Math.floor(baseHealth * healthMultiplier * typeData.healthMultiplier);
  const speed = Math.floor(baseSpeed * speedMultiplier * typeData.speedMultiplier);
...
// タワーを生成（タイプ別）
export function createTower(id: string, position: Position, type: TowerType): Tower {
...
// タワーの攻撃処理
export function towerAttack(tower: Tower, enemies: Enemy[], currentTime: number, activeBoosts?: any): {
  ...
  const boostedRange = tower.range + tower.tempBoosts.rangeBoost + (activeBoosts?.rangeBoost || 0);
  const boostedDamage = tower.damage + tower.tempBoosts.damageBoost + (activeBoosts?.damageBoost || 0);
  const speedBoostMultiplier = 1 + (tower.tempBoosts.speedBoost / 100) + (activeBoosts?.speedBoost || 0);
  const boostedAttackSpeed = tower.attackSpeed / speedBoostMultiplier;
  
  // 射程内の生きている敵を探す（ステルス敵は見えない場合がある）
  const targetEnemy = enemies.find(enemy => 
    enemy.isAlive && enemy.isVisible && calculateDistance(tower.position, enemy.position) <= boostedRange
  );
...
// 敵の特殊能力を処理
function processEnemyAbilities(enemy: Enemy, deltaTime: number): Enemy {
  if (!enemy.isAlive) return enemy;
  ...
  if (enemy.regenRate > 0 && currentTime - enemy.lastRegenTime >= 1000) {
    ...
    const newHealth = Math.min(enemy.maxHealth, enemy.currentHealth + regenAmount);
    updatedEnemy.currentHealth = newHealth;
    updatedEnemy.lastRegenTime = currentTime;
  }
  
  // ステルス能力（ランダムに見えたり見えなくなったり）
  if (enemy.type === 'stealth' && Math.random() < 0.03) { // 3%の確率で状態変更
...
function createDropItem(enemy: Enemy): DropItem | null {
  ...
  // アイテムタイプ決定
  const itemTypes: DropItemType[] = ['damage-boost', 'range-boost', 'speed-boost', 'gold-bonus', 'xp-bonus'];
...
export function updateGameState(gameState: GameState, deltaTime: number): GameState {
  ...
  // 敵の移動処理（速度倍率適用）
  updatedEnemies = updatedEnemies.map(enemy => moveEnemy(enemy, acceleratedDeltaTime));
  
  // 敵の特殊能力処理
  updatedEnemies = updatedEnemies.map(enemy => processEnemyAbilities(enemy, acceleratedDeltaTime));
  
  // 終点に到達した敵の処理
  const enemiesReachedEnd = updatedEnemies.filter(enemy => 
    !enemy.isAlive && enemy.position.x >= GAME_CONFIG.enemyPath[1].x
  );
  ...
  gameState.towers.forEach(tower => {
    ...
    if (canAttack) {
      const attackResult = towerAttack(tower, updatedEnemies, currentTime, gameState.activeBoosts);
  ...
  const defeatedEnemies = updatedEnemies.filter(enemy => 
    !enemy.isAlive && enemy.currentHealth <= 0 && !enemy.processed
  );
  ...
  defeatedEnemies.forEach(enemy => {
    ...
    const xpGain = Math.floor(baseXP * multiplier * gameState.activeBoosts.xpMultiplier);
    const goldGain = Math.floor(baseGold * multiplier * gameState.activeBoosts.goldMultiplier);
    ...
    if (enemyIndex !== -1) {
      updatedEnemies[enemyIndex] = { ...updatedEnemies[enemyIndex], processed: true };
    }
  });
  
  // 死んだ敵を除去（生きている敵のみ残す）
  updatedEnemies = updatedEnemies.filter(enemy => 
    enemy.isAlive && enemy.currentHealth > 0
  );
...
// タワーを配置
export function placeTower(gameState: GameState, position: Position, towerType: TowerType): GameState {
  if (!towerType) {
...
// ドロップアイテム収集
export function collectDropItem(gameState: GameState, itemId: string): GameState {
  ...
  switch (item.type) {
    ...
    case 'speed-boost':
      // タワー強化系は最も近いタワーに適用
      const nearestTower = findNearestTower(updatedGameState.towers, item.position);
      if (nearestTower) {
        updatedGameState = applyBoostToTower(updatedGameState, nearestTower.id, item);
      }
      break;
  }
...
// タワーにブーストを適用
function applyBoostToTower(gameState: GameState, towerId: string, item: DropItem): GameState {
...
export function endGame(gameState: GameState): void {
  const enemiesDefeated = gameState.enemies.filter(enemy => 
    !enemy.isAlive && enemy.currentHealth <= 0
  ).length;
...
// ゲーム速度を変更
export function setGameSpeed(gameState: GameState, speed: 1 | 2 | 3): GameState {
  return {
    ...gameState,
    gameSpeed: speed
  };
