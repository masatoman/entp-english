...
class ListeningGachaIntegration {
   ...
   */
  public async analyzeListeningPerformance(userId: string): Promise<ListeningAnalysis> {
    try {
      ...
      console.log(`🎯 リスニング分析完了: 弱い語彙${weakVocabulary.length}個, 推奨語彙${recommendedWords.length}個`);
      return analysis;
    } catch (error) {
  ...
  private async analyzePartPerformance(
    ...
    // 平均スコアを計算
    const totalScore = partSessions.reduce((sum, session) => sum + session.score, 0);
  ...
  private async generateRecommendedWords(
    ...
    try {
      ...
      weakVocabulary.forEach((weakWord) => {
        const relatedWords = allVocabulary.filter((word: VocabularyWord) => {
          if (usedWords.has(word.english) || word.english.toLowerCase() === weakWord) {
            return false;
          }
          
          // 同じカテゴリまたは関連するカテゴリ
          const isRelated = word.category === this.getCategoryForWord(weakWord) ||
                           this.isSemanticallyRelated(word.english, weakWord);
          
          return isRelated && word.level <= 3; // 中級まで
        });

        // 最大3個の関連語彙を追加
        relatedWords.slice(0, 3).forEach((word: VocabularyWord) => {
      ...
      Object.entries(partAnalysis).forEach(([part, analysis]: [string, any]) => {
        if (analysis.accuracy < 70) {
          const partRelatedWords = allVocabulary.filter((word: VocabularyWord) => {
            if (usedWords.has(word.english)) return false;
            
            // パートに応じた語彙カテゴリ
            const isPartRelated = this.isWordRelatedToPart(word, part);
            return isPartRelated && word.level <= 3;
          });

          // 最大2個のパート関連語彙を追加
          partRelatedWords.slice(0, 2).forEach((word: VocabularyWord) => {
            recommendedWords.push(word);
            usedWords.add(word.english);
          });
        }
      ...
      if (weakVocabulary.length < 5) {
        const toeicWords = allVocabulary.filter((word: VocabularyWord) => {
          if (usedWords.has(word.english)) return false;
          return word.category === 'business' || word.category === 'academic';
        });

        toeicWords.slice(0, 5).forEach((word: VocabularyWord) => {
          recommendedWords.push(word);
          usedWords.add(word.english);
        });
      }
  ...
  private getCategoryForWord(word: string): string {
    // 簡単なカテゴリ推定ロジック
    const businessWords = ['meeting', 'project', 'deadline', 'budget', 'report'];
  ...
  private isWordRelatedToPart(word: VocabularyWord, part: string): boolean {
    switch (part) {
      case 'part1':
        // Part 1: 写真描写 - 日常的な語彙
        return ['daily', 'transportation', 'food', 'clothing'].includes(word.category);
      case 'part2':
        // Part 2: 応答問題 - 会話的な語彙
        return ['daily', 'communication', 'social'].includes(word.category);
      case 'part3':
        // Part 3: 会話問題 - ビジネス・日常会話
        return ['business', 'daily', 'communication'].includes(word.category);
      case 'part4':
        // Part 4: 説明文問題 - アカデミック・ビジネス
        return ['business', 'academic', 'professional'].includes(word.category);
      default:
        return false;
    }
  }
   ...
   * ユーザーのリスニング分析結果を取得
   */
  public async getListeningAnalysis(userId: string): Promise<ListeningAnalysis | null> {
    try {
      const result = await dbManager.get(STORES.USER_PROGRESS, `listening_analysis_${userId}`);
   ...
   * リスニング学習後の推奨語彙を取得
   */
  public async getRecommendedWordsForGacha(userId: string): Promise<ListeningRecommendation[]> {
    try {
      ...
      if (!analysis || analysis.recommendedWords.length === 0) {
        ...
        return this.convertToRecommendations(newAnalysis.recommendedWords, newAnalysis);
      }
   ...
   * 推奨理由を生成
   */
  private generateRecommendationReason(word: VocabularyWord, analysis: ListeningAnalysis): string {
  ...
  private getWeakestPart(analysis: ListeningAnalysis): string {
    ...
    const weakestPart = parts.reduce((weakest, [part, data]) => {
      return data.accuracy < analysis.listeningPatterns[weakest as keyof typeof analysis.listeningPatterns].accuracy 
        ? part : weakest;
    }, 'part1');
...
// シングルトンインスタンスをエクスポート
export const listeningGachaIntegration = ListeningGachaIntegration.getInstance();
