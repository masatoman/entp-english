...
export class IntegratedAchievementsManager {
  ...
  private static getBestScore(userId: string): number {
    // 実際の実装では、最高スコアを取得
    return 750; // モック実装
  }

  private static getAttemptCount(userId: string, targetScore: number): number {
    // 実際の実装では、試行回数を取得
    return Math.floor(Math.random() * 10) + 1; // モック実装
  }

  private static checkPerfectRun(userId: string, testType: string): boolean {
    // 実際の実装では、完璧なテスト実行を確認
    return Math.random() > 0.9; // モック実装
  }

  private static getPerfectRunStreak(userId: string, testType: string): number {
    // 実際の実装では、連続完璧実行数を取得
  ...
  private static getCategoryAccuracy(userId: string, category: string): number {
