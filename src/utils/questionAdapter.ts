...
import { QuestionWithRank, Category, SkillField, QuestionRank } from '../types';
import { determineQuestionRank, calculateXPReward, CATEGORY_TO_SKILL_FIELD } from '../data/enhancedQuestions';
...
export function generateQuestionSet(
  allQuestions: Record<Category, Record<'easy' | 'normal' | 'hard', QuestionData[]>>,
  level: number,
  skillFieldAllocation: Record<SkillField, number>,
  ...
  for (const category of categories) {
    ...
    for (const difficulty of difficulties) {
      ...
      const enhancedQuestions = convertQuestionsToEnhanced(questions, category, difficulty, level);
      const filteredQuestions = filterQuestionsByLevel(enhancedQuestions, level, skillField);
      ...
      if (filteredQuestions.length > 0) {
        ...
        for (let i = 0; i < questionsToAdd; i++) {
          const selectedQuestion = selectWeightedQuestion(filteredQuestions, skillFieldAllocation);
          if (selectedQuestion) {
            selectedQuestions.push(adjustDifficultyForLevel(selectedQuestion, level));
          }
        }
      }
    }
