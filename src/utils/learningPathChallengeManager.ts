...
export class LearningPathChallengeManager {
    ...
    updatedChallenge.learningPath = updatedChallenge.learningPath.map(
      (step) => {
        ...
        switch (step.stepType) {
          ...
          case "vocabulary":
            const gachaData = GachaSystem.getUserGachaData(userId);
  ...
  private static grantChallengeRewards(
    ...
    if (rewards.specialEffects) {
      for (const effect of rewards.specialEffects) {
        this.applySpecialEffect(userId, effect);
      }
    }
  }
   ...
   * 特別効果を適用
   */
  private static applySpecialEffect(userId: string, effect: any): void {
    // 特別効果の適用ロジック（仮実装）
