...
export function getRequiredXPForLevel(level: number): number {
  ...
  const key = chapter * 20 - 19;
  const xpPerLevel =
    NEW_XP_CONFIG.LEVEL_XP_REQUIREMENTS[
      key as keyof typeof NEW_XP_CONFIG.LEVEL_XP_REQUIREMENTS
    ] || 50;
  return xpPerLevel;
}
...
export function calculateNewLevel(totalXP: number): UserLevel {
  ...
  const progress =
    ((totalXP - accumulatedXP) / getRequiredXPForLevel(level)) * 100;
...
export function calculateHeartSystem(level: number): HeartSystem {
  ...
  return {
    ...
    nextRecoveryTime: now + 5 * 60 * 1000,
    lastUsedTime: now,
  };
}
...
export function getRankProbabilities(chapter: Chapter) {
  const probabilities = {
    ...
    2: { normal: 0.6, rare: 0.3, epic: 0.09, legendary: 0.01 },
    3: { normal: 0.4, rare: 0.35, epic: 0.2, legendary: 0.05 },
    4: { normal: 0.25, rare: 0.35, epic: 0.3, legendary: 0.1 },
    5: { normal: 0.15, rare: 0.3, epic: 0.35, legendary: 0.2 },
  };
...
// ランク別XP計算
export function calculateRankXP(
  rank: QuestionRank,
...
// ステータス配分の検証
export function validateStatusAllocation(
  allocation: StatusAllocation
): boolean {
  ...
  return (
    total === 30 &&
    Object.values(allocation).every((value) => value >= 0 && value <= 30)
  );
}
...
export function processHeartRecovery(heartSystem: HeartSystem): HeartSystem {
  ...
  if (minutesElapsed >= 1) {
    ...
    const newCurrent = Math.min(
      heartSystem.current + heartsToAdd,
      heartSystem.max
    );
    const remainingTime =
      (minutesElapsed - Math.floor(minutesElapsed)) * (5 * 60 * 1000);
