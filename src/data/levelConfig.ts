import { Chapter, LevelConfig } from "../types";
...
for (let level = 1; level <= 100; level++) {
  ...
  if (level <= 20) {
    ...
    rankProbabilities = { normal: 0.6, rare: 0.3, epic: 0.09, legendary: 0.01 };
  } else if (level <= 60) {
    chapter = 3;
    requiredXP = 200;
    maxHearts = 3 + Math.floor((level - 1) / 10);
    rankProbabilities = { normal: 0.4, rare: 0.35, epic: 0.2, legendary: 0.05 };
  } else if (level <= 80) {
    chapter = 4;
    requiredXP = 300;
    maxHearts = 3 + Math.floor((level - 1) / 10);
    rankProbabilities = { normal: 0.25, rare: 0.35, epic: 0.3, legendary: 0.1 };
  } else {
    chapter = 5;
    requiredXP = 500;
    maxHearts = 3 + Math.floor((level - 1) / 10);
    rankProbabilities = { normal: 0.15, rare: 0.3, epic: 0.35, legendary: 0.2 };
  }
...
export function getChapterProgress(level: number): number {
  ...
  // const _chapterEndLevel = config.chapter * 20;
