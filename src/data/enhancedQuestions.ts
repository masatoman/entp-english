import { QuestionWithRank, Category, QuestionRank, SkillField } from '../types';
...
export function getRankProbability(rank: QuestionRank, level: number): number {
  if (level <= 20) {
    ...
    const probs = { normal: 0.6, rare: 0.30, epic: 0.09, legendary: 0.01 };
    return probs[rank];
  } else if (level <= 60) {
    const probs = { normal: 0.4, rare: 0.35, epic: 0.20, legendary: 0.05 };
    return probs[rank];
  } else if (level <= 80) {
    const probs = { normal: 0.25, rare: 0.35, epic: 0.30, legendary: 0.10 };
    return probs[rank];
  } else {
    const probs = { normal: 0.15, rare: 0.30, epic: 0.35, legendary: 0.20 };
    return probs[rank];
  }
}
