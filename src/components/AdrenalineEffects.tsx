...
import { AdrenalineEventData, AdrenalineSystem } from "../types/adrenalineSystem";
...
export default function AdrenalineEffects({ onEventTriggered }: AdrenalineEffectsProps) {
  const [system, setSystem] = useState<AdrenalineSystem>(adrenalineManager.getSystem());
  const [comboStatus, setComboStatus] = useState(adrenalineManager.getComboStatus());
  const [feverStatus, setFeverStatus] = useState(adrenalineManager.getFeverTimeStatus());
  const [pressureStatus, setPressureStatus] = useState(adrenalineManager.getPressureGaugeStatus());
  ...
  const triggerEvent = (event: AdrenalineEventData) => {
    ...
    // イベント表示を3秒後に削除
    setTimeout(() => {
      setActiveEvents(prev => prev.filter(e => e.timestamp !== event.timestamp));
    }, 3000);
  };
      ...
      {activeEvents.map((event) => (
        <Card 
          ...
          className={`
            ...
            ${event.type === "critical_hit" ? "border-yellow-400 bg-gradient-to-r from-yellow-50 to-orange-50" : ""}
            ${event.type === "combo_start" ? "border-red-400 bg-gradient-to-r from-red-50 to-pink-50" : ""}
            ${event.type === "fever_time_start" ? "border-purple-400 bg-gradient-to-r from-purple-50 to-indigo-50" : ""}
            ${event.type === "pressure_burst" ? "border-blue-400 bg-gradient-to-r from-blue-50 to-cyan-50" : ""}
          `}
        >
          <CardContent className="p-3">
            <div className="text-center">
              ...
              {event.effects.includes("critical_flash") && (
                <div className="text-xs text-yellow-600">✨ 画面が金色に光る ✨</div>
              )}
              ...
              {event.effects.includes("fever_background") && (
                <div className="text-xs text-purple-600">🎊 キラキラエフェクト 🎊</div>
              )}
            </div>
          </CardContent>
        </Card>
        ...
        <CardContent className="p-3 space-y-3">
          ...
          <div className="space-y-1">
            ...
            <Progress 
              ...
              className={`h-2 ${pressureStatus.canBurst ? "animate-pulse" : ""}`}
            />
          ...
          <div className="border-t pt-2">
            <div className="flex items-center justify-between">
              ...
              <Badge 
                ...
                className={`
                  text-sm font-bold
                  ${adrenalineManager.calculateTotalMultiplier() >= 3.0 ? "bg-gradient-to-r from-yellow-400 to-orange-500" : ""}
                  ${adrenalineManager.calculateTotalMultiplier() >= 2.0 ? "bg-gradient-to-r from-purple-400 to-pink-500" : ""}
                  ${adrenalineManager.calculateTotalMultiplier() >= 1.5 ? "bg-gradient-to-r from-blue-400 to-indigo-500" : ""}
                `}
              >
          ...
          <CardContent className="p-3">
            <div className="text-center">
              ...
              <div className="text-xs font-bold text-yellow-700">
                未開封の宝箱 {system.treasureBoxes.filter(box => !box.isOpened).length}個
              </div>
...
// XP計算ヘルパー
export const calculateAdrenalineXP = (baseXP: number, isCritical: boolean = false) => {
