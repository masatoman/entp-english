...
export function EnhancedVocabularyCard({ 
  ...
  const generateQuestions = () => {
    const vocabularyItems = getVocabularyByDifficulty(difficulty);
    const enhancedQuestions: VocabularyQuestion[] = vocabularyItems.map((item, index) => {
      const baseQuestion = convertQuestionToEnhanced(
        {
          ...
          choices: [item.word, item.word + 's', item.word + 'ed', item.word + 'ing']
        },
        'basic-grammar',
        'normal',
        userLevel.level
    ...
    // レベルに応じてフィルタリング
    const filteredQuestions = filterQuestionsByLevel(enhancedQuestions, userLevel.level, 'vocabulary');
          ...
          <Card className="mb-6">
            <CardHeader>
              <div className="flex items-center justify-between">
                <h1 className="text-2xl font-bold text-gray-800">語彙学習結果</h1>
              ...
              {userAnswer && (
                <div className="text-center space-y-4">
                  ...
                  <div className="bg-gray-50 p-4 rounded-lg">
                    ...
                    <p className="text-gray-700 mb-2">{currentQuestion.example}</p>
                    <p className="text-sm text-gray-600">{currentQuestion.exampleTranslation}</p>
                  </div>
                </div>
              )}
            </div>
