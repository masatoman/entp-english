...
export function VocabularyDifficultySelection({ onBack, onSelectDifficulty }: VocabularyDifficultySelectionProps) {
  const difficulties = [
    {
    ...
    },
    {
      ...
      difficulty: '上級',
      wordCount: '約30個の単語',
      examples: ['sophisticated', 'comprehensive', 'methodology', 'infrastructure']
    }
  ];
        ...
        <div className="space-y-4">
          {difficulties.map((difficulty) => (
            <Card 
              ...
              className={`cursor-pointer hover:bg-accent transition-colors active:scale-[0.98] ${difficulty.color}`}
              onClick={() => onSelectDifficulty(difficulty.level)}
            >
              <CardHeader className="pb-3">
                <div className="flex items-center justify-between">
                  <div>
                    <CardTitle className="text-xl">{difficulty.title}</CardTitle>
